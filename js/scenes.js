...
function buildBackdrop(scene, weather = "day") {
  scene.add.image(0, 0, "p_floor").setOrigin(0).setDisplaySize(GAME.W, GAME.H).setDepth(0);
  scene.add.rectangle(0, 0, GAME.W, GAME.H, 0xf6f8f9, 0.42).setOrigin(0).setDepth(2); // airy lift
  [[260, 200], [1000, 220], [640, 540], [420, 420]].forEach(([x, y]) => {
    scene.add.image(x, y, "fx_light").setDepth(3).setBlendMode(Phaser.BlendModes.ADD).setScale(2.2).setAlpha(0.1);
  });
  // drifting freight icons
  FREIGHT.forEach((f, i) => {
    const img = scene.add.image(-60, 120 + i * 110, "ic_box").setDepth(4).setTint(shadeNum(f.color, -0.2)).setAlpha(0.14).setDisplaySize(46, 46);
    scene.tweens.add({ targets: img, x: GAME.W + 60, duration: 18000 + i * 4000, repeat: -1, delay: i * 2600, ease: "Linear" });
  });
  // forklift driving across
  const fork = scene.add.sprite(-100, GAME.H - 120, "p_rc_operator", 1).setScale(PAINTED_SCALE.rc * 1.1).setDepth(5).setAlpha(0.95);
  ...
  if (wt.alpha > 0) scene.add.rectangle(0, 0, GAME.W, GAME.H, wt.color, wt.alpha * 0.7).setOrigin(0).setDepth(6);
  scene.add.image(0, 0, "fx_vignette").setOrigin(0).setDepth(7).setAlpha(0.25);
}
...
class MenuButton {
  ...
  draw() {
    ...
    g.clear();
    g.fillStyle(0x334155, 0.14); g.fillRoundedRect(-w / 2 + 2, -h / 2 + 4, w, h, 9);   // soft drop
    const fill = !this.enabled ? 0xe3e8eb : this.hover ? shadeNum(col, 0.78) : 0xffffff;
    g.fillStyle(fill, 0.97); g.fillRoundedRect(-w / 2, -h / 2, w, h, 9);
    g.lineStyle(2, this.enabled ? (this.hover ? shadeNum(col, -0.25) : 0x94a3ad) : 0xc3ccd1, 1);
    g.strokeRoundedRect(-w / 2, -h / 2, w, h, 9);
    g.fillStyle(col, this.enabled ? 1 : 0.35); g.fillRoundedRect(-w / 2, -h / 2, 4, h, 2);
    this.label.setColor(this.enabled ? CSS.text : CSS.muted);
    if (this.icon) this.icon.setTint(this.enabled ? shadeNum(col, -0.15) : 0x9fb0ba);
  }
...
class BootScene extends Phaser.Scene {
  ...
  preload() {
    // painted atlases (assets/painted*Data.js, embedded data URIs)
    Object.entries(window.PAINTED_META || {}).forEach(([name, m]) => {
      const src = (window.PAINTED_DATA || {})[name];
      if (!src) return;
      if (m.frames > 1) this.load.spritesheet(`p_${name}`, src, { frameWidth: m.frameWidth, frameHeight: m.frameHeight });
      else this.load.image(`p_${name}`, src);
    });
...
class SelectScene extends Phaser.Scene {
  ...
  card(sc, index, x, y, w, h) {
    ...
    glass(g, x, y, w, h, 12, unlocked ? 0xffffff : 0xe3e8eb, unlocked ? 0.95 : 0.75, unlocked ? 0x94a3ad : 0xc3ccd1, 1);
    const accent = [PAL.green, PAL.cyan, PAL.amber, PAL.purple, PAL.orange, PAL.red][index % 6];
    g.fillStyle(accent, unlocked ? 0.95 : 0.3); g.fillRoundedRect(x, y, 6, h, 3);
    ...
    for (let d = 0; d < 5; d++) { this.add.rectangle(x + 96 + d * 12, y + h - 25, 8, 8, d < sc.difficulty ? accent : 0xd7dde1).setDepth(21); }
    // stars
    this.add.text(x + w - 18, y + h - 30, starString(prog.stars), { fontFamily: FONTS.ui, fontSize: 18, color: prog.stars ? "#d97706" : "#c3ccd1" }).setOrigin(1, 0).setDepth(21);

    if (unlocked) {
      const hit = this.add.zone(x + w / 2, y + h / 2, w, h).setInteractive({ useHandCursor: true }).setDepth(22);
      hit.on("pointerover", () => { g.clear(); glass(g, x, y, w, h, 12, shadeNum(accent, 0.85), 0.97, accent, 1); g.fillStyle(accent, 0.95); g.fillRoundedRect(x, y, 6, h, 3); Sound.play("hover"); });
      hit.on("pointerout", () => { g.clear(); glass(g, x, y, w, h, 12, 0xffffff, 0.95, 0x94a3ad, 1); g.fillStyle(accent, 0.95); g.fillRoundedRect(x, y, 6, h, 3); });
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.on("pointerdown", () => Sound.unlock());
    this.startBanner();
    // event-driven celebration FX + audio moments (Phase 6, via the Phase 5 bus)
    this.sim.on("laneClean", ({ lane }) => { const p = lanePos(lane, this.sim.tune.stagingLanes); this.world.burst(p.x, p.y - 40, "green"); });
    this.sim.on("gpmPick", ({ lane }) => { const p = lanePos(lane, this.sim.tune.stagingLanes); this.world.burst(p.x, p.y - 20, "cyan"); });
    this.sim.on("trailerDocked", ({ side }) => { const d = LAYOUT.doors[side][0]; this.world.burst(d.x, 170, "amber"); });
    this.sim.on("ibtCleared", () => { const z = zoneOf("ibt"); this.world.burst(z.x, z.y - 20, "cyan"); Sound.play("trailer"); });
    this.sim.on("flowCritical", ({ on }) => { if (on) Sound.play("critical"); });
    if (this.tutorial) this.tut = new Tutorial(this, this.sim);
    this.events.on("shutdown", () => { Sound.stopAmbient(); });
  }

  startBanner() {
    const t1 = this.add.text(GAME.W / 2, GAME.H / 2 - 20, "SHIFT START", { fontFamily: FONTS.display, fontSize: 56, color: CSS.white, fontStyle: "700" }).setOrigin(0.5).setDepth(1500).setAlpha(0).setStroke("#f6f8f9", 6);
    const t2 = this.add.text(GAME.W / 2, GAME.H / 2 + 30, this.scenario.name, { fontFamily: FONTS.ui, fontSize: 20, color: CSS.amber, fontStyle: "700" }).setOrigin(0.5).setDepth(1500).setAlpha(0).setStroke("#f6f8f9", 4);
  ...
  onZoneClick(assignment) {
    ...
    if (assignment === "emptyTrailer") return this.sim.dispatch("restock");
    if (assignment === "raaPrimary") return this.sim.dispatch("raaPrep", { side: "primary" });
    if (assignment === "raaSecondary") return this.sim.dispatch("raaPrep", { side: "secondary" });
    if (assignment === "ibt") return this.sim.dispatch("clearIbt");
  }
  onBaseClick(base) {
    if (this.sim.state.paused || this.sim.state.gameOver) return;
    this.sim.dispatch("haul", { base });
  }
  togglePause() {
...
class ResultsScene extends Phaser.Scene {
  ...
  animateStars(cx, cy, stars) {
    for (let i = 0; i < 3; i++) {
      const s = this.add.text(cx + i * 44, cy, "★", { fontFamily: FONTS.ui, fontSize: 40, color: "#c3ccd1" }).setOrigin(0.5).setDepth(30);
      if (i < stars) this.time.delayedCall(300 + i * 280, () => { s.setColor("#d97706"); s.setScale(1.6); this.tweens.add({ targets: s, scale: 1, duration: 300, ease: "Back.out" }); Sound.play("star", i); });
    }
  }
  drawTimeline(x, y, w, h) {
    const data = this.result.timeline || [];
    const g = this.add.graphics().setDepth(30);
    glass(g, x, y, w, h, 8, 0xffffff, 0.8, 0x94a3ad, 0.8);
...
class GlossaryScene extends Phaser.Scene {
  ...
  create() {
    ...
    GLOSSARY.forEach((t, i) => {
      ...
      glass(g, x + 24, ty, w - 48, 52, 8, 0xffffff, 0.75, 0x94a3ad, 0.7);
      this.add.image(x + 50, ty + 26, `ic_${t.icon}`).setDisplaySize(24, 24).setTint(0xb45309).setDepth(2003);
