...
/* Tiny event bus (Phase 5). Presentation/audio SUBSCRIBE to one-shot moments
 * instead of polling state. Emitting mutates nothing — behavior-neutral. */
class SimEvents {
  constructor() { this._l = {}; }
  on(ev, fn) { (this._l[ev] = this._l[ev] || []).push(fn); return this; }
  off(ev, fn) { const a = this._l[ev]; if (a) this._l[ev] = a.filter((f) => f !== fn); return this; }
  emit(ev, payload) { const a = this._l[ev]; if (a) a.slice().forEach((f) => f(payload)); }
}
...
class WorkerManager {
  constructor(state, alerts, tune, events) {
    this.state = state; this.alerts = alerts; this.tune = tune; this.events = events;
    this.workers = [];
    const n = tune.staff;
  ...
  onArrive(w) {
    ...
    if (b.boxes >= cap) {
      b.fullPallet = { colorKey: b.colorKey, color: b.color, side: w.side }; b.blocked = true; Sound.play("beep");
      this.events.emit("palletFull", { side: w.side, index: b.index, colorKey: b.colorKey });
    }
...
class FlowManager {
  constructor(state, workers, scoring, alerts, tune, events) {
    this.state = state; this.workers = workers; this.scoring = scoring; this.alerts = alerts; this.tune = tune; this.events = events;
  }
  ...
  updateSide(side, delta) {
    ...
    if (side.refillTimer > 0) {
      ...
      if (side.refillTimer === 0) {
        ...
        this.events.emit("trailerDocked", { side: side.key });
      }
    }
  ...
  sourceReady(side) {
    ...
    if (!side.raaStarved) {
      ...
      this.events.emit("raaDry", { side: side.key });
    }
    return false;
  }
  hasSource(side) {
  ...
  emit(side) {
    ...
    // Each box is its OWN object. pickFreight returns the shared freight-table
    // entry; pushing it directly made claims color-wide (one worker per color,
    // ghost cartons everywhere). Explicit behavior fix 2026-07-04, see
    // SIM_CONTRACT.md changelog — goldens regenerated.
    side.line.push({ ...pickFreight(side.mix) });
  }

  generateOffer(side) {
    const cfg = this.tune.offerBlue;
    const bad = SimRNG.random() < cfg.badChance;
    const blue = bad ? rint(cfg.badMin, cfg.badMax) : rint(cfg.min, cfg.max);
    this.offerSeq = (this.offerSeq || 0) + 1;
    side.trailerOffer = { bluePct: blue, mix: makeOfferMix(blue), bad, id: this.offerSeq };
    Sound.play("beep");
    this.alerts.push(`${side.label}: trailer waiting — ${blue}% blue. Accept or reject.`, blue >= cfg.badMin ? "warn" : "info");
    this.events.emit("trailerOffer", { side: side.key, bluePct: blue, bad });
  }
  ...
  acceptOffer(side) {
    ...
    this.events.emit("trailerAccept", { side: side.key });
  }
  rejectOffer(side) {
    ...
    side.trailerOffer = null;
    this.events.emit("trailerReject", { side: side.key });
  }
  ...
  toggleSource(sideKey) {
    ...
    this.events.emit("sourceToggle", { side: sideKey, source: side.source });
  }

  updateFlowHealth(delta) {
    const W = this.tune.flowWeights; // named pressure model (values unchanged)
    ...
    const ibtP = this.state.ibt.count >= this.tune.ibtCapacity ? W.ibtFull : this.state.ibt.count >= W.ibtHighAt ? W.ibtHigh : 0;
    const srcP = (p.source === "ART" && p.artBacklog <= 0 ? W.artDry : 0) + (s.source === "ART" && s.artBacklog <= 0 ? W.artDry : 0);
    const idleP = (1 - this.workers.utilization()) * W.idleCrew;
    const pressure = lineBacklog * W.lineBacklog + full * W.fullPallet + blocked * W.blockedBase + needEmpty * W.needEmpty + mixed * W.mixedLane + ibtP + srcP + idleP;
...
class StagingManager {
  ...
  addToLane(lane, pallet) {
    ...
    if (lane.colorKey !== pallet.colorKey) {
      ...
      this.events.emit("laneMixed", { lane: lane.index });
    }
    ...
    if (lane.pallets.length >= this.tune.stagingCapacity && !lane.mixed) {
      ...
      this.events.emit("laneClean", { lane: lane.index, hot, colorKey: lane.colorKey });
    }
  }
  ...
  updateGpm(delta) {
    ...
    this.state.staging.lanes.forEach((lane) => {
      if (lane.ready && !lane.mixed && SimRNG.random() < this.tune.gpmChance) {
        ...
        this.events.emit("gpmPick", { lane: lane.index, cleared });
      }
  ...
  updateIbt(delta) {
    ...
    if (ibt.timer === 0) {
      const cleared = ibt.count;
      this.state.stats.ibtCleared += cleared; this.alerts.push(`IBT cleared ${cleared} blue pallets.`, "good");
      ibt.count = 0; ibt.clearing = false;
      this.events.emit("ibtCleared", { cleared });
    }
  }
  requestIbt() {
    const ibt = this.state.ibt;
    ...
    this.events.emit("ibtStart", { count: ibt.count });
  }
  ...
  manualGpm() {
    ...
    this.events.emit("manualGpm", { cleared: staged });
  }
  ...
  reworkMixedLane() {
    ...
    this.events.emit("rework", { groups: groups.length });
  }
...
class RcController {
  constructor(state, staging, scoring, alerts, tune, events) {
    this.state = state; this.staging = staging; this.scoring = scoring; this.alerts = alerts; this.tune = tune; this.events = events;
  }
  ...
  targetFor(job) {
    if (job.type === "haul" || job.type === "partial") {
      // v2.2 layout: approach from the station's outer side so the forklift
      // stays out of the workers' pick corridor (explicit geometry change,
      // see SIM_CONTRACT.md changelog — goldens regenerated)
      if (job.phase === "pickup") return rcBasePos(job.base.sideKey, job.base.index);
  ...
  onArrive() {
    ...
    if (j.type === "haul" || j.type === "partial") {
      ...
      this.events.emit("haul", { dest: j.dest, lane: j.lane ? j.lane.index : null, partial: j.type === "partial" });
      this.endJob(); return;
    }
  ...
  finishDwell() {
    ...
    if (j.type === "raaPrep") {
      ...
      if (side.raaPrep < max) {
        ...
        this.alerts.push(`${side.label} RAA pallet prepped.`, "good");
        this.events.emit("raaPrep", { side: j.sideKey });
      }
      this.endJob();
    }
  }
...
class Sim {
  constructor(scenario, seed) {
    if (seed != null) SimRNG.seed(seed); // harness-only; the game never passes a seed
    ...
    this.events = new SimEvents();
    this._critical = false;
    this.alerts = new AlertManager();
    this.scoring = new ScoringManager(this.state, this.alerts, this.tune);
    this.workers = new WorkerManager(this.state, this.alerts, this.tune, this.events);
    this.workers.scoring = this.scoring;
    this.flow = new FlowManager(this.state, this.workers, this.scoring, this.alerts, this.tune, this.events);
    this.staging = new StagingManager(this.state, this.workers, this.scoring, this.alerts, this.tune, this.events);
    this.rc = new RcController(this.state, this.staging, this.scoring, this.alerts, this.tune, this.events);
  }

  on(ev, fn) { return this.events.on(ev, fn); }
  off(ev, fn) { return this.events.off(ev, fn); }

  /* Single facade over the full RC decision set (Phase 5). The UI routes every
   * action through here; the underlying methods below remain the frozen API.
   * Denial reasons still surface via sim.alerts (unchanged behavior). */
  dispatch(action, payload = {}) {
    switch (action) {
      case "haul": return this.rc.requestHaulAt(payload.base);
      case "togglePartial": this.state.partialArmed = !this.state.partialArmed; return this.state.partialArmed;
      case "raaPrep": return this.rc.requestRaaPrep(payload.side);
      case "restock": return this.rc.requestRestock();
      case "toggleSource": return this.flow.toggleSource(payload.side);
      case "acceptTrailer": return this.flow.acceptOffer(this.state.sides[payload.side]);
      case "rejectTrailer": return this.flow.rejectOffer(this.state.sides[payload.side]);
      case "clearIbt": return this.staging.requestIbt();
      case "rework": return this.staging.reworkMixedLane();
      case "manualGpm": return this.staging.manualGpm();
      case "reassign": return this.workers.reassign(payload.to);
  ...
  update(delta) {
    ...
    const crit = s.flowHealth < this.tune.flowCritical;
    if (crit !== this._critical) { this._critical = crit; this.events.emit("flowCritical", { on: crit }); }
    if (s.shiftRemaining <= 0 && !s.gameOver) { s.gameOver = true; Sound.play("shiftEnd"); this.events.emit("shiftEnd", {}); }
