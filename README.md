...
The game is self-contained — all sprites and icons are embedded as data URIs, so you
can open `index.html` directly in a modern browser. Only Phaser, the web fonts, and
the icon CDN load online the first time. You can also serve the folder:
...
- **Worker sprites:** one sheet, `assets/worker_sprites_v6.png` — 4 characters × 18 frames (front idle 0–2, side-walk 3–9, back 10–12, carton-carry 13–17). Embedded as a data URI (`assets/workerSpritesData.js`) so the game runs from `file://` too. Animated at runtime: directional facing (`flipX` for left), multi-frame walk cycles, and the carry-walk when hauling a box. The raw AI sheet is processed offline with Pillow (flood-fill the baked white background + a 2-pass light-fringe trim to kill the halo, then each gap-separated figure is repacked feet-centered/feet-aligned into a uniform grid). **Forklift sprites:** `assets/rc_operator_sprites.png`, embedded as a data URI (`assets/rcOperatorSpritesData.js`).
